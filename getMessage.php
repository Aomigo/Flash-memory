...
updateChat(1); // one because we got only one game

function updateChat($gameID) 
{
    $pdo = getPDO();
    $try = $pdo->prepare("SELECT m.*, u.picture FROM message m LEFT JOIN user u ON m.user_id=u.id 
                            WHERE (m.created_at > NOW()-INTERVAL 1 DAY) AND m.game_id = ?");
    $try->execute([$gameID]);
